...
type Dimension  = "account_name" | "bank";
...
const palette = [
  ...
  "#06b6d4", "#f97316", "#ec4899", "#84cc16", "#3b82f6",
  "#14b8a6", "#a855f7",
];
...
function bucket(accounts: SavingsAccount[], dim: Dimension, ownerFilter: OwnerFilter): Slice[] {
  const ownerKey = ownerFilter === "Vadym" ? "vadym" : "lisa";
  const filtered = accounts.filter((a) => (a.owner ?? "vadym").toLowerCase() === ownerKey);
  const totals: Record<string, number> = {};
  for (const a of filtered) {
    const key = dim === "account_name" ? a.account_name : a.bank;
...
function ratioSlices(accounts: SavingsAccount[]): Slice[] {
  const totals: Record<string, number> = { Vadym: 0, Lisa: 0, Joint: 0 };
  for (const a of accounts) {
    const o = (a.owner ?? "vadym").toLowerCase();
    const key = o === "lisa" ? "Lisa" : o === "joint" ? "Joint" : "Vadym";
    const v = Number(a.balance_gbp ?? 0);
    if (Number.isFinite(v) && v > 0) totals[key] += v;
  }
  const total = Object.values(totals).reduce((a, b) => a + b, 0);
  return Object.entries(totals)
    .filter(([, v]) => v > 0)
    ...
    .map(([name, value]) => ({
      ...
      value,
      pct: total > 0 ? (value / total) * 100 : 0,
      color: RATIO_COLORS[name] ?? palette[0],
    }));
}
    ...
    <div className="grid grid-cols-1 sm:grid-cols-5 gap-4 items-center">
      <div className="sm:col-span-2 relative h-56">
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
          <div className="text-[10px] uppercase tracking-wide text-gray-400 dark:text-gray-500">{centerLabel}</div>
...
export default function SavingsAllocationChart({ accounts }: { accounts: SavingsAccount[] }) {
  ...
  const hasData     = chartData.length > 0 && chartTotal > 0;

  const ratioData   = useMemo(() => ratioSlices(accounts), [accounts]);
  const ratioTotal  = useMemo(() => ratioData.reduce((acc, d) => acc + d.value, 0), [ratioData]);
  const hasRatio    = ratioData.length > 0 && ratioTotal > 0;
    ...
    <>
      ...
      <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4 sm:p-6 shadow-sm">
        ...
        {!hasData ? (
          ...
          <DonutChart data={chartData} total={chartTotal} centerLabel="Total" />
        )}
      </div>
      ...
      <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4 sm:p-6 shadow-sm">
        <div className="mb-4">
          <h2 className="text-base font-semibold text-gray-900 dark:text-gray-50">Savings Ratio</h2>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
            Total split by Vadym / Lisa / Joint
          </p>
        </div>
        {!hasRatio ? (
          <div className="h-56 flex items-center justify-center text-sm text-gray-500 dark:text-gray-400 text-center px-4">
          ...
          <DonutChart data={ratioData} total={ratioTotal} centerLabel="Total" />
        )}
      </div>
    </>
  );
