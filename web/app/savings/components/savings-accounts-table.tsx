...
type OwnerFilter = "All" | "Vadym" | "Lisa" | "Joint";
const OWNER_FILTERS: OwnerFilter[] = ["All", "Vadym", "Lisa", "Joint"];
...
function ownerBadge(owner: string) {
  const o = (owner ?? "vadym").toLowerCase();
  const isJoint = o === "joint";
  const isLisa  = o === "lisa";
  const cls = isJoint
  ...
  const label = isJoint ? "Joint" : isLisa ? "Lisa" : "Vadym";
  return (
...
export default function SavingsAccountsTable({ accounts }: { accounts: SavingsAccount[] }) {
  ...
  const filtered = useMemo(() => {
    if (ownerFilter === "All") return sorted;
    const target = ownerFilter.toLowerCase() === "vadym" ? "vadym" : ownerFilter.toLowerCase();
    return sorted.filter((a) => (a.owner ?? "vadym").toLowerCase() === target);
  }, [sorted, ownerFilter]);
        ...
        <table className="w-full text-sm">
          <thead className="border-b border-gray-200 dark:border-gray-800">
            <tr>
              ...
              {ownerFilter === "All" && <Th label="Owner" k="owner" />}
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
            {filtered.map((a) => (
              <tr key={a.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                ...
                {ownerFilter === "All" && <td className="px-3 py-2.5">{ownerBadge(a.owner)}</td>}
